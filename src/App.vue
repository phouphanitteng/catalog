...
<template>
  <div style="background: #e4e9f7; font-family: 'Julee', cursive;">
    <div class="bg bg-info px-2 py-2 text-white sticky-top top-0" style="font-family: 'Julee', cursive;">
      <h4 class="container">Where are your country?</h4>
    </div>
    <div class="container rounded bg-white d-flex justify-content-between" style="position: sticky; top: 3.35rem;">
      <div class="w-25 my-4">
        <div class="input-group">
          <input class="form-control border-5 rounded" type="text" placeholder="Search by Country name"
            v-model="searchTerm" id="example-search-input">
        </div>
      </div>
      <div class="my-4">
        <button class="btn btn-primary me-2 " @click="sort('name.official')">Sort Ascending </button>
        <button class="btn btn-primary" @click="sort('name.official'); sortDirection = 'desc'">Sort
          Descending</button>
      </div>
    </div>
    <div class="container container-s mt-1">
      <div v-for="country in paginatedCountries" :key="country.cca3" class="w-100 content bg-white rounded">
        ...
        <a class="btn p-0" @click="openModal(country)"> <strong> Official Name: </strong>{{ country.name.official }}</a>
        ...
        <div><strong> IDD:</strong> <span v-for="(s, index) in country.idd.suffixes"><span v-if="index <= 3">
              {{ country.idd.root }} {{ s }} {{ "\n" }} </span></span> </div>
      </div>
    </div>
